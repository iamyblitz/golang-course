...
import (
	...
	"repo-stat/api/internal/adapter/subscribe"
...
func NewHandler(ctx context.Context, log *slog.Logger, cfg config.Config) (http.Handler, error) {
	...
	subscribeClient, err := subscribe.NewClient(cfg.Services.Subscribe, log)
	if err != nil {
		log.Error("cannot init subscribe adapter", "error", err)
		return nil, err
	}

	pingUseCase := usecase.NewPing(processorClient, subscriberClient)
	repositoryUseCase := usecase.NewRepository(processorClient)
	subscriptionUseCase := usecase.NewSubscription(subscribeClient)

	mux := http.NewServeMux()
	AddRoutes(mux, log, pingUseCase, repositoryUseCase, subscriptionUseCase)
